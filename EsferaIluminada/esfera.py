...
void main(void) 
{
    ...
    vec3 objectColor = texture(textureSlot, texturaObjeto).xyz;
...
def malha():
    ...
    aTextura = coordenadaTextura(M, N)
    ...
    GL.glBufferData(GL.GL_ARRAY_BUFFER, len(aPosicao)*aPosicao.itemsize,
                    ctypes.c_void_p(aPosicao.buffer_info()[0]), GL.GL_STATIC_DRAW)
    ...
    GL.glBufferData(GL.GL_ARRAY_BUFFER, len(aNormal)*aNormal.itemsize,
                    ctypes.c_void_p(aNormal.buffer_info()[0]), GL.GL_STATIC_DRAW)
    GL.glVertexAttribPointer(
        1, 3, GL.GL_FLOAT, GL.GL_FALSE, 0, ctypes.c_void_p(0))

    VBO_cor = GL.glGenBuffers(1)
    GL.glBindBuffer(GL.GL_ARRAY_BUFFER, VBO_cor)
    GL.glBufferData(GL.GL_ARRAY_BUFFER, len(aTextura)*aTextura.itemsize,
                    ctypes.c_void_p(aTextura.buffer_info()[0]), GL.GL_STATIC_DRAW)
    GL.glVertexAttribPointer(
        2, 2, GL.GL_FLOAT, GL.GL_FALSE, 0, ctypes.c_void_p(0))
...
def loadTexture(filename):
    ...
    GL.glTexImage2D(GL.GL_TEXTURE_2D, 0, modo, width, height, 0,
                    modo, GL. GL_UNSIGNED_BYTE, data)
...
def desenha():
    ...
    camera = glm.vec3(radius * math.cos(a), 2, radius * math.sin(a))
    model = glm.mat4(1.0)
    view = glm.lookAt(camera, glm.vec3(5, 0, 0), glm.vec3(0, 1, 0))

    GL.glBindVertexArray(malhaVAO)
    GL.glUseProgram(progId)
    GL.glUniformMatrix4fv(GL.glGetUniformLocation(
        progId, "model"), 1, GL.GL_FALSE, glm.value_ptr(model))
    GL.glUniformMatrix4fv(GL.glGetUniformLocation(
        progId, "view"), 1, GL.GL_FALSE, glm.value_ptr(view))
    GL.glUniformMatrix4fv(GL.glGetUniformLocation(
        progId, "projection"), 1, GL.GL_FALSE, glm.value_ptr(projection))
    GL.glDrawElements(GL.GL_TRIANGLE_STRIP, tamIndice,
